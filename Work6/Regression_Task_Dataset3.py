...
class Dataset(torch.utils.data.Dataset):
    def __init__(self):
        ...
        X = np.array(pd.read_csv(".\Stress-Lysis.csv"))

        self.X = np.array(X[:, 1:]).astype(np.float32)
        X_std = stand(self.X)
        self.X = torch.FloatTensor(X_std)

        self.Y = np.array(X[:, 0]).astype(np.float32)
        Y_std = stand(self.Y)
...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.layers = torch.nn.Sequential(
            torch.nn.Linear(in_features=3, out_features=24),
            BatchNorm(in_features=24),
            torch.nn.ReLU(),
