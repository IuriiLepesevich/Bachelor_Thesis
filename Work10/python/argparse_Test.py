...
parser.add_argument('-sequence_name', default=f'reg4', type=str)
...
class BatchNormSimple(torch.nn.Module):
    def __init__(self, in_features: int):
        ...
        self.train_mean_sum = np.zeros((in_features,))

        self.train_n = 0
        self.train_var_sum = 0
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            self.train_mean_sum += self.mean_train.cpu().data.numpy()
            self.train_var_sum += self.var_train.data.numpy()
            norm = (self.x - self.mean_train) / (torch.sqrt(self.var_train + self.eps))
            self.output = self.gamma * norm + self.beta
        else:
            self.mean_test = self.mean_train_sum / self.train_n
            self.var_test = self.train_var_sum / self.train_n
...
class BatchNormRunningMean(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if self.var_test is None:
                self.var_test = var_train.detach()
                self.mean_test = mean_train.detach()
            else:
                self.mean_test = self.alpha * self.mean_test + (1 - self.alpha) * mean_train.detach()
                self.var_test = self.alpha * self.var_test + (1 - self.alpha) * var_train.detach()
