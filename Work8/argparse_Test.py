...
parser.add_argument('-batch_mode', default=f'Last', type=str)
parser.add_argument('-run_name', default=f'run', type=str)
parser.add_argument('-sequence_name', default=f'reg', type=str)
...
batch_mode = args.batch_mode # Last, Simple(doesn't work), RunningMean
...
#Here is my unsuccessful attempt to implement BatchNormSimple class
...
class BatchNormSimple(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if mode == 'train':
            ...
            self.mean_test = torch.mean(tf.convert_to_tensor(self.mean_train_sum), axis=0)
            self.var_test = torch.mean(tf.convert_to_tensor(self.var_train_sum), axis=0)
...
class BatchNormRunningMean(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if mode == 'train':
            ...
            self.mean_test = torch.mean(self.x, axis=0)
            self.var_test = torch.mean(self.x, axis=0)
            self.mean_test = self.gamma * self.mean_test + (1 - self.alpha) * self.mean_train
            self.var_test = self.gamma * self.var_test + (1 - self.alpha) * self.var_train
...
class ModelLast(torch.nn.Module):
    def __init__(self):
        super().__init__()
...
class ModelSimple(torch.nn.Module):
    def __init__(self):
        ...
        self.layers = torch.nn.Sequential(
            torch.nn.Linear(in_features=10 + 2 * len(dataset_full.labels), out_features=24),
            torch.nn.ReLU(),
            BatchNormSimple(in_features=24),
            torch.nn.Linear(in_features=24, out_features=16),
            torch.nn.ReLU(),
            BatchNormSimple(in_features=16),
            torch.nn.Linear(in_features=16, out_features=1)
        )
...
class ModelLastRunningMean(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.layers = torch.nn.Sequential(
            torch.nn.Linear(in_features=10 + 2 * len(dataset_full.labels), out_features=24),
            torch.nn.ReLU(),
            BatchNormRunningMean(in_features=24),
            torch.nn.Linear(in_features=24, out_features=16),
            torch.nn.ReLU(),
            BatchNormRunningMean(in_features=16),
            torch.nn.Linear(in_features=16, out_features=1)
        )
...
if batch_mode == 'Last':
    model = ModelLast()
elif batch_mode == 'Simple':
    model = ModelSimple()
elif batch_mode == 'RunningMean':
    model = ModelLastRunningMean()
